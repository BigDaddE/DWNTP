...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_proposals_received{chaincode=~"dwntp.*"}[20s]))|
    \item \textbf{Mechanism:} Calculates the per-second rate of incoming transaction requests over a 20-second rolling window. The \texttt{sum()} function aggregates the traffic hitting all peers across the network into a single global metric.
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_successful_proposals{chaincode=~"dwntp.*"}[20s]))|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|avg(rate(ledger_transaction_count{validation_code="VALID", chaincode=~"dwntp.*"}[20s]))|
    \item \textbf{Mechanism:} Calculates the per-second rate of valid transactions permanently written to the ledger. The query explicitly filters for the \texttt{dwntp} chaincode to exclude internal system transactions. The \texttt{avg()} function is utilized rather than \texttt{sum()} because every peer in the distributed network commits the exact same blocks; \texttt{avg()} calculates the true network consensus throughput without artificially multiplying the count or hiding lagging peers as \texttt{max()} would.
...
\paragraph{Endorsement Latency Over Time}
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_proposal_duration_sum{chaincode="dwntp",success="true"}[20s])) / sum(rate(endorser_proposal_duration_count{chaincode="dwntp",success="true"}[20s]))|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_proposal_failures{chaincode=~"dwntp.*"}[20s]))| and \verb|sum(rate(ledger_transaction_count{validation_code="MVCC_READ_CONFLICT", chaincode=~"dwntp.*"}[20s]))|
    \item \textbf{Mechanism:} Directly queries Fabric for endorsement failures and utilizes the ledger transaction count to track Multi-Version Concurrency Control (MVCC) conflicts.
...
\begin{itemize}
    \item \textbf{Expression:} \verb|rate(broadcast_enqueue_duration_sum[20s]) / rate(broadcast_enqueue_duration_count[20s])|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|rate(ledger_blockstorage_commit_time_sum[20s]) / rate(ledger_blockstorage_commit_time_count[20s])|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|rate(ledger_statedb_commit_time_sum[20s]) / rate(ledger_statedb_commit_time_count[20s])|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(process_cpu_seconds_total[20s])) by (job, instance) * 100|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|process_resident_memory_bytes|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(gossip_comm_messages_sent[20s])) by (instance)|
