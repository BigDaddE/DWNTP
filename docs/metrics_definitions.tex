...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_proposals_received[20s]))|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_successful_proposals[20s]))|
...
\begin{itemize}
    \item \textbf{Expression:} \verb|sum(rate(endorser_proposal_failures[20s]))| and \verb|sum(rate(ledger_transaction_count{validation_code="MVCC_READ_CONFLICT", chaincode=~"dwntp.*"}[20s]))|
